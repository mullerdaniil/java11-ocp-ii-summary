...
- у enum есть static метод ```values()```, методы ```name()``` и ```ordinal()```
```java
...
- в switch нельзя писать enum полностью, нужно только имя
```java
...
- аннотация - это форма метаданных, добавляемых в код, но не являющася частью программы. Аннотации похожи на интерфейс, однако могут применяться к классам, к методам, к выражениям, к другим аннотациям. 
...
- есть 4 главных интерфейса в Java Collections Framework (при этом все из этих кроме ```Map``` наследуются от интерфейса ```Collections```):
    - ```List```
    - ```Set```
    - ```Queue```
    - ```Map```
...
- ```Collections.sort()``` принимает только объекты, реализующие ```Comparable```, однако вместе со коллекцией можно передать ```Comparator```.
